...
import numpy as np
import os.path
class EnviromentalBenefitsResults2(Module):
	def __init__(self):
	...
	def run(self):
		...
		print str(1)+","+str(self.FF[0])+","+str(self.VR[0])+","+str(self.FV[0])+","+str(self.WQ[0])
		if os.path.exists(self.tmpFile):
			f = open(self.tmpFile,'a')
			f.write(str(1)+","+str(self.FF[0])+","+str(self.VR[0])+","+str(self.FV[0])+","+str(self.WQ[0])+"\n")		
			f.close()
		else:

...
void Microclimate_heat::printRaster(DM::RasterData * r)
{
...
void Microclimate_heat::fillZeros(DM::RasterData * r)
{
...
double Microclimate_heat::chooseTab(double perc)
{
...
double Microclimate_heat::calcLST(QList<QList<double> > t)
{
    int pointer;
    double res;
...
QList<QList<double> > Microclimate_heat::readWsud(QString filename)
{
...
QList<double> Microclimate_heat::getTechAreasForCell(int x, int y,double w, QList<QList<double> > table)
{
    QList<double>zero;
    zero.append(0);
    //double res = 0;
...
void Microclimate_heat::exportMCtemp(DM::RasterData *r, QString filename, double scale)
{
    int counter = 0;
    QFile file (filename);
    if(file.open(QIODevice::WriteOnly))
...
double Microclimate_heat::getCoef()
{
    double res=0;
    QFile file;
...
double Microclimate_heat::getTempForSurface(int surface, int percentile)
{
...
DM::RasterData * Microclimate_heat::calcReductionAirTemp(DM::RasterData *r)
{
    int counter;
    double value;
...
double Microclimate_heat::calcOverlay(double x1,double y1,double g1,double x2,double y2, double g2)
{
...
double Microclimate_heat::calcA(QRectF *r)
{
    return r->width() * r->height();
}

std::vector<QPointF> Microclimate_heat::getCoveringCells(double x, double y, double g1,double g2)
{
...
bool Microclimate_heat::createInputDialog()
{
    QWidget * w = new p8microclimate_heat_gui(this);
    w->show();
    return true;
}
